...
public:
	...
	void set_initial_data(double t0, double r0, double th0, double phi0) {
		...
		//v_r[0] = 0.0;
		//v_th[0] = sqrt(-V_eff(E, L, mass, r[0], th[0],angmom) * (g_rr(mass,r[0],th[0],angmom) / g_thth(r[0],th[0], angmom)));
	...
	void RK4(double tolerance, double init_dtau, double dtausmall, double dtaubig) {
		double tau = 0;
		...
		while (tau > taufinal) {
			...
			/*
			if (r + (2825.0 / 27648.0) * r_k1 + (18575.0 / 48384.0) * r_k3 + (13525.0 / 55296.0) * r_k4 + (277.0 / 14336.0) * r_k5 + (1.0 / 4.0) * r_k6 <= r_event_max) {
				inspiral = true;
				break;
			}

			// Order 5 values
			t = t + (2825.0 / 27648.0) * t_k1 + (18575.0 / 48384.0) * t_k3 + (13525.0 / 55296.0) * t_k4 + (277.0 / 14336.0) * t_k5 + (1.0 / 4.0) * t_k6;
			r = r + (2825.0 / 27648.0) * r_k1 + (18575.0 / 48384.0) * r_k3 + (13525.0 / 55296.0) * r_k4 + (277.0 / 14336.0) * r_k5 + (1.0 / 4.0) * r_k6;
			th = th + (2825.0 / 27648.0) * th_k1 + (18575.0 / 48384.0) * th_k3 + (13525.0 / 55296.0) * th_k4 + (277.0 / 14336.0) * th_k5 + (1.0 / 4.0) * th_k6;
			phi = phi + (2825.0 / 27648.0) * phi_k1 + (18575.0 / 48384.0) * phi_k3 + (13525.0 / 55296.0) * phi_k4 + (277.0 / 14336.0) * phi_k5 + (1.0 / 4.0) * phi_k6;
...
/*
			...
			if ((abs(r - r_ad) > tolerance || abs(th - th_ad) > tolerance) && (dtau / 2.0) > (dtausmall)) {
				dtau = dtau / 2.0;
			}

			else if ((abs(r - r_ad) <= tolerance && abs(th - th_ad) <= tolerance) && (dtau * 2.0) < (dtaubig)) {
				dtau = dtau * 2.0;
			}
*/
		}
	...
	void save_to_file() {
		std::ofstream myFile("kerr_ray.txt");
	...
	int colour_coder() {
		...
		if (phi>0){
			phi_r = fabs(phi) - floor(fabs(phi)/(2.0*M_PI))*2.0*M_PI;
		}
		else{
			phi_r = 2.0*M_PI - ( fabs(phi) - floor(fabs(phi)/(2.0*M_PI))*2.0*M_PI );
		}
		...
		/*
			...
			if (y[size - 1] < 0 && z[size - 1]>0) {
				colour=1;
			}
			else if (y[size - 1] > 0 && z[size - 1] > 0) {
				colour=2;
			}
			else if (y[size - 1] < 0 && z[size - 1] < 0) {
				colour=3;
		...
		*/
			...
			if (0< th && th<M_PI_2 && 0<phi_r && phi_r<M_PI){
				colour = 2;
			}
			else if (0<th && th<M_PI_2 && M_PI<phi_r && phi_r<2*M_PI){
				colour = 1;
			}
			else if (M_PI_2 <th && th<M_PI && 0<phi_r && phi_r<M_PI){
				colour = 4;
			}
			else if (M_PI_2 <th && th<M_PI && M_PI<phi_r && phi_r<2*M_PI){
				colour = 3;
			}
		}
        return colour;
...
int main() {
	...
	//Ray-trace photons backwards from the observer in a loop

	//this is for the way I do the parametrization
	/*
	double* y = new double[100];
	double* z = new double[100];

	linspace(-20.0, 20.0, 100, y);
	linspace(-20.0, 20.0, 100, z);
	
	std::ofstream myFile("kerr_shadow_data_2.txt");
	Kerr_potential_test_photon photon(M, a,0.0,0.0,100);
	double phi;
	double theta;
	double r;
	double E;
	...
	for (int i = 0; i < 100; i++) {
		for (int j = 0; j < 100; j++) {
			r = sqrt(pow(x,2) + pow(y[i],2) + pow(z[j],2));
			theta = acos(z[j]/r);
			phi = atan(y[i] / x);
			
			E = - (-1.0 + (-1.0)*(-sqrt(pow(y[i],2) + pow(z[j],2))/pow(r,2))*(-1.0));
			L = (-1.0) + sqrt(pow(y[i],2) + pow(z[j],2));

			//E = 1.0;
			//L = sqrt(pow(y[i],2) + pow(z[j],2));
			photon.E = E;
			photon.L=L;
			...
			photon.set_initial_data(0.0, r, theta, phi);
			//do the RK-4 and error analysis
			photon.RK4(pow(10,-13), pow(10,-2),pow(10,-4),pow(10,-1));
			myFile <<r<<","<<phi<<","<<theta<<"," << y[i] << "," << z[j] << "," << photon.colour_coder() << ","<<photon.x[photon.size-1]<< ","<<photon.energy_checker/photon.size <<"\n";
			//std::cout<< j <<"\n";
		}
		std::cout << i << "\n";
	}
	...
	*/
	...
	linspace(-M_PI/3.0, M_PI/3.0, 100, alpha);
	linspace(-M_PI/3.0, M_PI/3.0, 100, beta);

	std::ofstream myFile("flat_space_data_1.txt");
	Kerr_potential_test_photon photon(M, a,0.0,0.0,-50);
	double x;
	double y;
	double phi = 0.0;
	double th = M_PI_2;
	...
	for (int i = 0; i < 100; i++) {
		for (int j = 0; j < 100; j++) {
			...
			E = (1.0/A_t) - P_phi_obs*g_tphi(M,r,th,a)/sqrt(g_phiphi(M,r,th,a));
			...
			//do the RK-4 and error analysis
			photon.RK4(pow(10,-13), pow(10,-2),pow(10,-4),pow(10,-1));
			myFile << x << "," << y << "," << photon.colour_coder() << ","<<photon.r<< ","<<photon.energy_checker/photon.size <<"\n";
			//std::cout<< j <<"\n";
		}
		std::cout << i << "\n";
	}
	...
	/*
	Kerr_potential_test_photon test_1(M, a, 1.0 / 3.0, 1.0,-100);

	//Initial Conditions for a spherical orbit. E and L were set to produce spherical orbit.
	test_1.set_initial_data(0.0, 20.0, 0.35, M_PI+0.35);
	
	//Do the RK4 and check error
	test_1.RK4_energy_error(pow(10,-1),-pow(10,-2),pow(10,-3),pow(10,-1));
	std::cout << test_1.colour << " "<<test_1.x[test_1.size-1] << " "<< test_1.y[test_1.size-1]<<" "<<test_1.z[test_1.size-1] << "\n";
	*/
	//saving the ray to file
	//test_1.save_to_file();
