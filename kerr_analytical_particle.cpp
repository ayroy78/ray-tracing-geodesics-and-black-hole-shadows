...
public:
	...
	void RK4 (double tol, double init_dtau, double dtausmall, double dtaubig) {
		...
		while (tau < taufinal) {
			...
			carter_constant = carter_constant + fabs(pow(sigma(r[it],th[it],angmom)*v_th,2) + pow(cos(th[it]),2)*(pow(angmom,2)*(1.0 - pow(E,2))+ pow(L/sin(th[it]),2)));
...
*/
	...
	void save_to_file() {
		std::ofstream myFile("kerr_particle_Lm2.txt");
...
int main(){
	...
	//Do the RK4
	particle.RK4(pow(10,-13), pow(10,-2), pow(10,-2), pow(10,-2));
    std::cout<< "The average deviation from expected zero is: "<< particle.energy_checker/particle.x.size() <<"\n";
	std::cout<< "The starting value of the Carter Constant was: "<< particle.Q <<"\n";
	std::cout<< "The numerical value of the average Carter Constant was: "<< particle.carter_constant/particle.x.size() <<"\n";
	std::cout<< "The deviation in the Carter Constant was: "<< particle.Q - particle.carter_constant/particle.x.size() <<"\n";
	std::cout<< "The size of the iteration was: "<< particle.x.size() << "\n";
    std::cout <<"The ending radial position is: "<<particle.r[particle.r.size()-1]<<"\n";
